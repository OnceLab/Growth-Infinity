...
class RanLife {
    constructor(base_angle,speed = 1,stroke = 80, weight = 1, step = [60,-60], ...options) {
        ...
        this.hidden_first_lines_movements = false;

        this.step = step;
        this.branch_angle_base = int(random(0, 2)) ? 45 : -45;
    ...
    update() {
        ...
        if (this.hidden_first_lines_movements)
        {
            stroke(0);
        } else {
...
function setup() {
    ...
    for (let i = 0; i < n; i++ ) {
        ...
        complex.push(new RanLife(0 + 45 * i,(i+1)*1.5,2.5,i*.3,[45,-45]));
    }
}
